...
function getLogLevel(): LogLevel {
  // Browser-safety hotfix: guard all process reads with comprehensive fallbacks
  const lvl = ((globalThis as any).__APP_LOG_LEVEL) ?? (import.meta?.env?.VITE_LOG_LEVEL) ?? (process?.env?.LOG_LEVEL) ?? 'info';
  const level = lvl;
 ...
 * Format log entry
 */
function formatLogEntry(level: string, message: string, meta?: any, error?: Error): any {
 ...
 * Write log entry
 */
function writeLog(level: LogLevel, levelName: string, message: string, meta?: any, error?: Error): void {
