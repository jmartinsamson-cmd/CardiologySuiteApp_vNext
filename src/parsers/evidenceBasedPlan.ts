...
/**
 * Clinical action with evidence class
 */
...
/**
 * Lab result with value and metadata
 */
interface LabResult {
  name: string;
  value: number | string;
...
/**
 * Diagnosis entry (string or object)
 */
type DiagnosisEntry = string | { diagnosis: string; [key: string]: unknown };

/**
 * Parsed clinical note data
 */
 ...
 */
export const CLINICAL_GUIDELINES: Record<string, GuidelineProtocol> = {
  'STEMI': {
 ...
 */
export function generateEvidenceBasedPlan(parsed: ParsedNote): string | null {
 ...
 */
function checkLabContraindication(contraindication: string, labs?: LabResult[]): boolean {
  ...
  if (contraLower.includes('hyperkalemia') || contraLower.includes('potassium')) {
    ...
    if (kLab && typeof kLab.value === 'number' && kLab.value > 5.5) return true;
  }
  ...
  if (contraLower.includes('thrombocytopenia') || contraLower.includes('platelet')) {
    ...
    if (pltLab && typeof pltLab.value === 'number' && pltLab.value < 50) return true;
  }

  if (contraLower.includes('renal') || contraLower.includes('crcl')) {
    const crLab = labs.find(l => l.name?.toLowerCase().includes('creatinine'));
    if (crLab && typeof crLab.value === 'number' && crLab.value > 3.0) return true; // Severe renal dysfunction
  }

  if (contraLower.includes('bleeding')) {
    const hgbLab = labs.find(l => l.name?.toLowerCase().includes('hemoglobin'));
    if (hgbLab && typeof hgbLab.value === 'number' && hgbLab.value < 7) return true; // Active bleeding suspected
  }
 ...
 * @returns True if contraindicated
 */
function checkVitalContraindication(contraindication: string, vitals?: Vitals): boolean {
 ...
 */
export function getGuidelineReference(condition: string): GuidelineProtocol | null {
 ...
 */
export function listAvailableGuidelines(): string[] {
