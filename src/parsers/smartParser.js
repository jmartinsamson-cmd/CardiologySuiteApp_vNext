...
import { extractVitals, extractMeds, extractAllergies, extractDiagnoses, extractProvider, extractDemographics, extractLabs, } from "./entityExtraction.js";
/**
...
export function parseNote(rawText) {
    ...
    debugLog(`parse: conf=${confidence.toFixed(2)} sections=${Object.keys(sections).length} warnings=${warnings.length} (${duration}ms)`);
    return {
        data,
        warnings,
        confidence,
...
function detectBySignals(text) {
    ...
    for (const para of paragraphs) {
        ...
        for (const [canonical, signals] of Object.entries(SIGNAL_WORDS)) {
            const matchCount = signals.filter((sig) => para.toLowerCase().includes(sig.toLowerCase())).length;
...
function extractEntities(sections, fullText) {
    ...
    if (!vitals.source ||
        ...
        if (objectiveVitals.source === "vitals-table" ||
...
function calculateConfidence(data, warnings, sections) {
    ...
    const criticalWarnings = warnings.filter((w) => w.includes("CRITICAL")).length;
...
//# sourceMappingURL=smartParser.js.map