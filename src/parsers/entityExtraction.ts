 ...
 */
export function extractClinicalContext(text: string): ClinicalContext {
 ...
 */
export function disambiguateDiagnoses(
  diagnoses: Array<string | { diagnosis: string }>,
  ...
  const disambiguated: DiagnosisWithConfidence[] = processed.map(dx => {
    let confidence = 1.0;
    ...
    const relatedCausality = context.causality.find(c => 
      c.effect && dx.toLowerCase().includes(c.effect.toLowerCase())
    );
  ...
  if (/VITAL SIGNS.*MIN.*MAX/i.test(text)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^Temp\s+Min:/i.test(line)) {
        ...
        if (tempMatch) vitals.temp = Number.parseFloat(tempMatch[1]);
 ...
 */
export function extractMeds(text: string): string[] {
  if (!text) return [];
...
/**
 * Keyword pattern for diagnosis extraction
 */
 ...
 */
export function extractDiagnoses(text: string): string[] {
  if (!text) return [];
 ...
 */
function extractLastValue(line: string, pattern: RegExp): number | null {
 ...
 */
export function extractLabs(text: string): Labs {
  if (!text) return {};
